...
use onboarding::{OnboardingIntention, AI_FEATURES, HELIOS_CLOUD_FEATURES};
...
impl LoginSlideView {
    ...
    fn render_skip_dialog(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let is_terminal = matches!(self.intention, OnboardingIntention::Terminal);
        let title_text = if is_terminal {
            "Are you sure you want to disable Helios Drive?"
        } else {
        ...
        let body_text_str = if is_terminal {
            "Helios Drive lets you save workflows and knowledge across devices and share them with your team. By continuing, you won't have access to the following features:"
        } else {
            "Helios is better with AI. By continuing, you won't have access to any of the following features:"
        };
        let body_text =
