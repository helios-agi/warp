...
fn main() {
    ...
    if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("macos") {
        // Metal shader compilation requires Xcode.app (not just Command Line Tools).
        // When using the wgpu renderer, we skip Metal shader compilation entirely
        // since wgpu uses WGSL shaders compiled at runtime.
        let use_wgpu = env::var("CARGO_FEATURE_EXPERIMENTAL_WGPU_RENDERER").is_ok();
        if !use_wgpu {
            bindgen_shader_types();
            compile_metal_shaders();
        } else {
            // The metal Rust module still compiles unconditionally (it is referenced by
            // RendererManager regardless of renderer backend), but it embeds the shader
            // artifacts via compile-time include_bytes!/include!.  Generate lightweight
            // stub files so those macros resolve without needing Xcode.
            generate_metal_shader_stubs();
        }
        compile_objc_lib();
    }
}

fn generate_metal_shader_stubs() {
    // When building with the wgpu renderer, Metal shader artifacts are not compiled.
    // However, metal/renderer.rs uses include_bytes!("shaders.metallib") and
    // include!("shader_types.rs") at compile time, so we must produce placeholder files.
    // The Metal renderer is never instantiated at runtime when wgpu is active
    // (Device::new() always returns Device::WGPU when cfg(wgpu) is true).
    let out_path = PathBuf::from(env::var("OUT_DIR").unwrap());

    // Empty .metallib — include_bytes! resolves to &[] and is never loaded at runtime.
    let metallib = out_path.join("shaders.metallib");
    std::fs::write(&metallib, b"").expect("failed to write stub shaders.metallib");

    // Minimal shader_types.rs — provides the type definitions that metal/renderer.rs
    // references via `include!()`.  Types are sized to match what clang/bindgen would
    // produce for the simd vector types on macOS (vector_float2 ≅ u64,
    // vector_float4 ≅ u128).
    let shader_types_rs = out_path.join("shader_types.rs");
    std::fs::write(
        &shader_types_rs,
        r#"// Stub shader types — generated by build.rs for wgpu renderer builds.
// The Metal renderer is not used at runtime when building with wgpu.
#[allow(non_camel_case_types)]
pub type vector_float2 = u64;
#[allow(non_camel_case_types)]
pub type vector_float4 = u128;
...
#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct PerRectUniforms {
    pub origin: vector_float2,
    pub size: vector_float2,
    pub corner_radius_top_left: f32,
    pub corner_radius_top_right: f32,
    pub corner_radius_bottom_left: f32,
    pub corner_radius_bottom_right: f32,
    pub border_top: f32,
    pub border_right: f32,
    pub border_bottom: f32,
    pub border_left: f32,
    pub background_start: vector_float2,
    pub background_end: vector_float2,
    pub background_start_color: vector_float4,
    pub background_end_color: vector_float4,
    pub border_start: vector_float2,
    pub border_end: vector_float2,
    pub border_start_color: vector_float4,
    pub border_end_color: vector_float4,
    pub icon_color: vector_float4,
    pub is_icon: ::std::os::raw::c_int,
    pub drop_shadow_offsets: vector_float2,
    pub drop_shadow_color: vector_float4,
    pub drop_shadow_sigma: f32,
    pub drop_shadow_padding_factor: f32,
    pub dash_length: f32,
    pub gap_lengths: vector_float2,
}

#[repr(C)]
#[derive(Debug, Copy, Clone)]
pub struct PerGlyphUniforms {
    pub origin: vector_float2,
    pub size: vector_float2,
    pub uv_left: f32,
    pub uv_top: f32,
    ...
    pub color: vector_float4,
    pub is_emoji: ::std::os::raw::c_int,
}
"#,
    )
    .expect("failed to write stub shader_types.rs");
}
